...
namespace Remact.Net.Internal
{
  ...
  public class WcfBasicService
  {
    ...
    private object ConnectPartner(ActorInfo client, ActorMessage req, out WcfBasicServiceUser svcUser)
    {
      svcUser = null;
      if (req.ClientId != 0)
    ...
    /// <param name="svcUser">Output the user object containing a "ClientIdent.UserContext" object for free application use</param>
    /// <returns>Service info as response</returns>
    private object DisconnectPartner(ActorInfo client, ActorMessage req, out WcfBasicServiceUser svcUser)
    {
      int i = req.ClientId - m_FirstClientId;
    ...
    internal bool FindPartnerAndCheck (ActorMessage req, out WcfBasicServiceUser svcUser)
    {
      int i = req.ClientId - m_FirstClientId;
      if (i >= 0 && i < ServiceIdent.InputClientList.Count)
      {
    ...
    internal object CheckBasicResponse(ActorMessage req, out WcfBasicServiceUser svcUser)
    {
      ...
      if (cltReq != null)
      {
        ...
        switch (cltReq.Usage)
        {
        case ActorInfo.Use.ClientConnectRequest:    response = ConnectPartner    (cltReq, req, out svcUser); break;
        case ActorInfo.Use.ClientDisconnectRequest: response = DisconnectPartner (cltReq, req, out svcUser); break;
